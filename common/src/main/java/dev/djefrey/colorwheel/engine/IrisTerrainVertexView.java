...
{
    public static final long STRIDE = 12 // Position
                                    +  4 // Color
                                    +  8 // UV0
                                    +  4 // UV1
                                    +  4 // Normal + Padding
    ...
    {
        if (dst instanceof ClrwlVertexView clrwlView)
        {
            MemoryUtil.memCopy(ptr + srcIndex * STRIDE, clrwlView.ptr() + dstIndex * ClrwlVertexView.STRIDE, STRIDE);
    ...
    public void write(MutableVertexList dst, int srcStartIndex, int dstStartIndex, int vertexCount)
    {
        if (dst instanceof ClrwlVertexView clrwlView)
        {
            ...
            {
                var srcIndex = srcStartIndex + i;
                var dstIndex = dstStartIndex + i;

                MemoryUtil.memCopy(ptr + srcIndex * STRIDE, clrwlView.ptr() + dstIndex * ClrwlVertexView.STRIDE, STRIDE);
                dst.overlay(dstIndex, overlay(i));
            }
